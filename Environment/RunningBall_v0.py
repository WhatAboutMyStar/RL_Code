...
class RunningBall:
    ...
    def step(self, action):
        ...
        pygame.image.save(self.screen, "./data/1.png")
        next_state = cv2.imread("./data/1.png")
        next_state = cv2.cvtColor(next_state, cv2.COLOR_BGR2RGB)
    ...
    def reset(self):
        ...
        pygame.draw.circle(surface=self.screen, color=[0, 255, 0], center=[self.enemy_x, self.enemy_y], radius=20,
                           width=0)
        pygame.image.save(self.screen, "./data/test.png")

        next_state = cv2.imread("./data/test.png")
        next_state = cv2.cvtColor(next_state, cv2.COLOR_BGR2RGB)
        next_state = next_state.astype('float32')
        next_state = np.transpose(next_state, axes=(2, 0, 1))
